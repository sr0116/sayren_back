            ...
            listval.forEach((listkey2, listval2) -> {
              if(listval2.split(",").length > 1){
                for(String t : listval2.split(",")){
                  tagCrawlRepository.save(Tag.builder().product(productCrawlRepository.findByModelName(listkey).get()).tagName(listkey2).tagValue(t).build());
                }
              }
              else {

                tagCrawlRepository.save(Tag.builder().product(productCrawlRepository.findByModelName(listkey).get()).tagName(listkey2).tagValue(listval2).build());
              }
            });
          }
        }
