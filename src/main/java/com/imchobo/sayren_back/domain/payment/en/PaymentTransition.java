...
public enum PaymentTransition {
  ...
  public static PaymentTransition fromPortOne(PaymentInfoResponse info, Long expectedAmount) {
    ...
    if ("paid".equalsIgnoreCase(status)) {
      if (!info.getAmount().equals(expectedAmount)) {
        log.warn("결제 금액 불일치: 요청 금액={}, PortOne 금액={}", expectedAmount, info.getAmount());
        return FAIL_SYSTEM; // 금액 불일치는 시스템 오류로 처리
      }
      return COMPLETE;
    }
    // PortOne의 신규 상태 대응
    ...
    if ("partialcancelled".equalsIgnoreCase(status)) {
    ...
    if ("failed".equalsIgnoreCase(status)) {
      ...
      log.warn("PortOne 결제 실패 매핑: status={}, errorCode={}, errorMsg={}", status, errorCode, errorMsg);
      // 그 외 → 시스템 장애 처리
      return FAIL_SYSTEM;
    }

    // 기타 예상 못한 상태
    log.error("PortOne 알 수 없는 상태: status={}, errorCode={}, errorMsg={}",
