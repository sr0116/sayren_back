...
public class ImageUtil {
  public static byte[] convertUrlToWebpBytes(String imageUrl) throws Exception {
    ...
    if (originalImage == null) {
      throw new IllegalArgumentException("이미지 로드 실패: " + imageUrl);
    }

    // 2. WebP Writer 준비
    Iterator<ImageWriter> writers = ImageIO.getImageWritersByMIMEType("image/webp");
    if (!writers.hasNext()) {
      throw new IllegalStateException("WebP ImageWriter 없음 - webp-imageio 의존성 확인 필요");
    }
    ...
    if (webpBytes == null || webpBytes.length == 0) {
      throw new RuntimeException("WebP 변환 결과가 비어있음: " + imageUrl);
    }
